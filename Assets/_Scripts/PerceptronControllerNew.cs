...
public class PerceptronControllerNEW : MonoBehaviour
{
    ...
    public XORCombiner xorCombiner;
    ...
    private void Awake () {
        ...
        if (xorCombiner != null) {
            xorCombiner.OnTrainingCompleted += OnTrainingCompleted;
        }
    }
    ...
    private void CalculateAndLogOutput () {
        if (xorCombiner != null) {
            double output = xorCombiner.CalcOutput(input1, input2);
            Debug.Log("Полученное значение из XORCombiner: " + output);
